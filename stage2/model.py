...
class Down(nn.Module):
    def __init__(self, in_ch: int, out_ch: int) -> None:
        super().__init__()
        self.block = nn.Sequential(ResBlock(in_ch, out_ch), ResBlock(out_ch, out_ch))
...
class Up(nn.Module):
    def __init__(self, in_ch: int, skip_ch: int, out_ch: int) -> None:
        ...
        self.block = nn.Sequential(ResBlock(out_ch + skip_ch, out_ch), ResBlock(out_ch, out_ch))
...
class UNetRes(nn.Module):
    """
    U-Net + ResBlocks, SiLU activation.
    入力: (B, 3, H, W)
    出力: (B, 1, H, W) logits
    """
